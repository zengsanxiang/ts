...
const getCityDistricts = () => {
  let c = [];
  for (const element of city.city_data) {
...
async function getOrCreateRegion(
  ...
  const [exist] = await db
    .select()
    .from(regions)
    .where(and(eq(regions.name_cn, nameCN), eq(regions.level, level)))
    .limit(1);
...
const getLocations = async (district: any, refresh_index: number) => {
  try {
    const body = {
      filter_opt: {
        ...district,
    ...
    const res = await axios.post(
      "https://api5-normal-sicily-hl.faceu.mobi/sicily/v2/poi/combine/list/",
      body,
      {
        params: query,
        timeout: 5000,
    ...
    for (let item of poi_combine_list) {
      ...
      const provinceId = await getOrCreateRegion(
        "",
        province,
        "province",
        countryId
      ...
      const districtId = await getOrCreateRegion(
        "",
        district,
        "district",
        cityId
      ...
      // 🏞️ 检查 location 是否已存在
      const [existLoc] = await db
        .select()
        .from(locations)
        .where(eq(locations.o_id, o_id))
        .limit(1);
      ...
      // 🏠 插入新 location
      const locId = nanoid();
      let p = await db.insert(locations).values({
        o_id,
        id: locId,
        name_cn,
        ad_cn,
